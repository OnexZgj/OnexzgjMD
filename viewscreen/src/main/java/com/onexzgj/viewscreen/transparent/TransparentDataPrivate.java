...
import com.onexzgj.viewscreen.accessibility.WrapperAccessibilityDelegate;
 ...
 * @author onexzgj
 */
public class TransparentDataPrivate {
     ...
     * @param activity
     */
    private static void addSensorsDataTrackLayout(Activity activity) {
        View decorView = activity.getWindow().getDecorView();
        if (decorView!=null){
            if (decorView instanceof ViewGroup){
                TransparentAppClickOverlayLayout topLayout = new TransparentAppClickOverlayLayout(activity);

                ((ViewGroup) decorView).addView(topLayout,new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
                ViewCompat.setElevation(topLayout,999F);
            }
        }
    }
    ...
    public static void trackViewOnClick(View view) {
        try {
            ...
            jsonObject.put("$element_id", TransparentDataPrivate.getViewId(view));
            jsonObject.put("$element_content", TransparentDataPrivate.getElementContent(view));

            Activity activity = TransparentDataPrivate.getActivityFromView(view);
            if (activity != null) {
